...
class BeanstalkDeployCommand(BeanstalkWindowCommand):
    ...
    def run(self, repository):
        ...
        thread = thread_type(
            self.window, self.repository, self.on_preparing_done)
        thread.start()
    ...
    def on_preparing_done(self, environment, revision, message=''):
        def start_release():
            ...
            thread = thread_type(self.window, self.repository,
                                 environment['id'], revision, message,
                                 self.on_releasing_done)
            thread.start()
            ThreadProgress(thread, "Releasing %s" % revision, "Done")

        msg = "Are you sure want to deploy revision %s to %s?" % (
